      ...
      <q-card>
        <q-card-section>
          ...
          <div
            ...
            class="q-mt-xs q-gutter-sm"
          >
            <q-badge
              v-for="(role, index) in marketplaceStore?.userRoles"
              :key="index"
            >
              {{ formatRole(role) }}
            </q-badge>
...
export default defineComponent({
  ...
  setup() {
    ...
    const userImage = computed(() => {
      if (marketplaceStore.user.profilePictureUrl)
        return marketplaceStore.user.profilePictureUrl;
    ...
    async function logOut() {
      try {
        ...
        await backend
          .post(`users/revoke_token/`, undefined, { customConfig })
          .catch(console.error)
