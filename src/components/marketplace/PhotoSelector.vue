...
import {
  ...
  dataUrlToFile,
  resizeImage,
} from "src/marketplace/chat/attachment";
import {
  checkPermissions,
  requestPermissions,
  getPhoto,
  ...
  setup(props, { emit: $emit }) {
    ...
    watch(innerVal, (newVal, oldVal) => {
      ...
      if (newVal instanceof File)
        newVal.objectUrl = URL.createObjectURL(newVal);
    });
    ...
    async function checkOrRequestCameraPermissions() {
      ...
      const request =
        promptStatuses.includes(permission.photos) ||
        promptStatuses.includes(permission.camera);
      if (request) permission = await requestPermissions();
      return {
        camera: permission.camera === "granted",
        photos:
          permission.photos === "photos" || permission.photos === "granted",
      };
    }
    ...
    function selectPhoto(evt) {
      return getPhotoFromCamera().catch((error) => {
        console.error(error);
