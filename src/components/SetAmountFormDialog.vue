              ...
              >
                <template v-slot:prepend>
                  <img
                    ...
                    :fallback-src="
                      convertIpfsUrl(selectedCurrency?.imageUrl, 1)
                    "
                  ...
                  <img
                    ...
                    class="q-mr-xs"
                  />
                  <span v-else-if="selectedCurrencyFlag">{{
                ...
                <template v-slot:option="ctx">
                  ...
                  >
                    ...
                    >
                      ...
                      />
                      <span
                        v-else-if="ctx?.opt?.id?.startsWith('fiat/')"
                        style="font-size: 30px; line-height: 30px"
                      >
                        {{
                          getCountryFlagEmoji(
                            getCountryCodeFromCurrency(ctx?.opt?.symbol)
                          )
                        }}
                      </span>
                    </q-item-section>
                    <q-item-section>
                      ...
                      <q-item-label v-if="ctx.opt?.name" caption>{{
                        ctx?.opt?.name
                      }}</q-item-label>
                    </q-item-section>
                  </q-item>
  ...
  setup(props) {
    ...
    const cryptoCurrencyOpts = computed(() => {
      ...
      if (Array.isArray(props.currencies))
        opts = [...props.currencies].map(parseAsset).filter(Boolean);
      // Only add initial currency if it's not a fiat currency
      if (
        initialCurrency &&
        !initialCurrency?.id?.startsWith?.("fiat/") &&
        !opts.find((asset) => asset?.id === initialCurrency?.id)
      ) {
    ...
    const filteredCurrencyOpts = computed(() => {
      let filtered = cryptoCurrencyOpts.value.filter(
        (asset) => !asset?.id?.startsWith?.("fiat/")
      );
      if (props.hideInvalidOptions)
        filtered = filtered.filter((asset) => asset?.valid !== false);
      return filtered;
    });
    ...
    const allCurrencyOpts = computed(() => {
      ...
      const fiat =
        fiatCurrencyOpts.value.find((f) => f.symbol === preferredFiat) ||
        fiatCurrencyOpts.value.find((f) => f.symbol === "PHP") ||
        fiatCurrencyOpts.value[0];
    ...
    const isSelectedCurrencyToken = computed(() =>
      selectedCurrency.value?.id?.startsWith?.("ct/")
    );
    const isFiatSelected = computed(() =>
      selectedCurrency.value?.id?.startsWith?.("fiat/")
    );

    const selectedCurrencyFlag = computed(() => {
      if (!isFiatSelected.value || !selectedCurrency.value?.symbol) return null;
      const countryCode = getCountryCodeFromCurrency(
    ...
    const tokenFiatRate = computed(() => {
      ...
      const tokenCategory = paymentCurrency.value?.id?.replace("ct/", "");
      return marketStore.getTokenFiatRate(
        tokenCategory,
        selectedCurrency.value.symbol
      );
    });
    ...
    function handleAmountBlur() {
      if (
        fiatAmountValue.value !== null &&
        fiatAmountValue.value !== undefined
      ) {
        // Format according to locale for display
        const decimals = amountFieldDecimals.value || 0;
        fiatAmountDisplay.value = formatNumberAutoDecimals(
          fiatAmountValue.value,
          decimals
        );
      }
    }
    ...
    async function fetchConversionRates() {
      ...
      try {
        ...
        console.error(
          "[SetAmountFormDialog] Error fetching conversion rates",
          error
        );
    ...
    async function updateAmount(amount) {
      ...
      if (isFiatSelected.value) {
        ...
        if (isUsingDirectTokenFiatRate.value) {
          paymentAmount =
            Math.round(amount * fiatToPaymentRate.value * factor) / factor;
        } else {
          paymentAmount =
            Math.round((amount / fiatToPaymentRate.value) * factor) / factor;
        }
    ...
    function checkAmount() {
      ...
      const paymentAsset = isFiatSelected.value
      ...
      if (assetId?.startsWith?.("ct/"))
        tokenCategory = assetId.replace("ct/", "");
...
body.body--dark .disabled-button {
