...
import {
  HEARTBEAT_REQUEST_HEADER,
  useMarketplaceHeartbeatStore,
} from "src/stores/marketplace-heartbeat";
...
export const TOTP_AUTH_BASE_SECRET_KEY =
  process.env.MARKETPLACE_AUTH_TOTP_BASE_SECRET;

export const backend = axios.create({
  baseURL:
    process.env.MARKETPLACE_BASE_URL || "https://commercehub.paytaca.com/api",
...
backend.interceptors.response.use(null, async (error) => {
  if (error?.response?.status == 401)
    await setAuthToken(undefined).catch(console.error);
...
export async function getAuthToken() {
  try {
    const authToken = await SecureStorage.get({
...
export async function setAuthToken(value = "") {
  try {
    if (value === undefined) {
      const removeResp = await SecureStorage.remove({
        key: AUTH_TOKEN_STORAGE_KEY,
      });
      return { success: removeResp.value };
    }

    const setResponse = await SecureStorage.set({
      key: AUTH_TOKEN_STORAGE_KEY,
      value: value,
    });
...
const heartbeat = {
  parseForRequest(config, token) {
    ...
    const isLastHeartbeatExpired =
      marketplaceHeartbeatStore.isLastHeartbeatExpired();
  ...
  parseResponse(response) {
    ...
    const marketplaceHeartbeatStore = useMarketplaceHeartbeatStore();
    const responseHeartbeatTimestamp = parseFloat(
      response?.headers?.["x-heartbeat-timestamp"]
    );
    const lastHeartbeatTimestamp = Number.isFinite(responseHeartbeatTimestamp)
      ? responseHeartbeatTimestamp
      : Date.now();
