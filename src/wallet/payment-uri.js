 ...
 * @param {Object} opts
 * @param {Boolean} opts.keepParsedParams
 * 
 * {@link https://gist.github.com/khirvy019/27ea9baae16c37bb49e6a477dbb46054}
 */
export function parsePaytacaPaymentUri(paymentUri = '', opts) {
  const response = {
  ...
  if (!opts?.keepParsedParams) delete searchParams.asset

  if (searchParams?.name) {
    response.name = searchParams?.name
    if (!opts?.keepParsedParams) delete searchParams?.name
  }

  if (searchParams?.message) {
    response.message = searchParams?.message
    if (!opts?.keepParsedParams) delete searchParams?.message
  }
  ...
  if (searchParams?.POS) {
    ...
    if (!opts?.keepParsedParams) delete searchParams.POS
  }
  ...
  if (response.asset.chain === 'main') {
    ...
    addressValidator = address => String(address).match(/^0x[a-fA-F0-9]{40}$/)
  }
