...
class PaymentIndexValidator {
  ...
  static MAX_POSID_ON_MAX_PAYMENT_INDEX =
    this.MAX_UNHARDENED_ADDRESS_INDEX % this.MAX_POS_DEVICE_ID_COUNT;
  ...
  static validate(paymentIndex, posId) {
    ...
    try {
      ...
      if (
        paymentIndex == this.MAX_PAYMENT_INDEX &&
        posId > MAX_POSID_ON_MAX_PAYMENT_INDEX
      ) {
...
export class Wallet {
  ...
  set xPubKey(value) {
    this._xPubKey = value || null;
    this._mainHDNode = null;
    if (!this._xPubKey) return;
    try {
  ...
  getAddressSetAt(index) {
    if (!this.mainHDNode) return null;
   ...
   */
  async generateReceivingAddress(
    paymentIndex = 1,
    opts = { skipSubscription: true }
  ) {
    const validation = Wallet.paymentIndexValidator.validate(
      paymentIndex,
      this.posId
    );
  ...
  async getTransactions(opts) {
    ...
    try {
      ...
      if (opts?.type === "incoming" || opts?.type === "outgoing")
        queryParams.type = opts.type;

      pageKey = JSON.stringify(
        Object.assign({ walletHash: this.walletHash }, queryParams)
      );
